...
while web_camera.isOpened():
    ...
    face = face_cascade.detectMultiScale(
        Image, scaleFactor=1.1, minNeighbors=4)
    for(x, y, w, h) in face:
        ...
        if pred == 1:
            ...
            cv2.putText(Image, 'STOP!! NO MASK', ((x+w)//2, y+h+20),
                        cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 3)
        else:
            cv2.rectangle(Image, (x, y), (x+w, y+h), (0, 255, 0), 3)
            cv2.putText(Image, 'MASK FOUND', ((x+w)//2, y+h+20),
                        cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 3)
        datet = str(datetime.datetime.now())
        cv2.putText(Image, datet, (410, 460),
